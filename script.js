...
<div class="ep-item" onclick="openArticle(${articles.indexOf(a)})">
  ...
  document.getElementById('topics-cloud').innerHTML = tags.map(t => `<div class="tag-pill" onclick="searchByTag('${t}')">${t}</div>`).join('');
...
window.addEventListener('scroll', () => {
  ...
  document.getElementById('reading-bar').style.width = Math.min(100, p) + '%';
}, { passive: true });

/* ══════════════════════════════════════════════════
   GLOBAL CLOSE
══════════════════════════════════════════════════ */
function closeAll() { closeArticle(); closeAdmin(); currentFilter = 'all'; renderHome(); window.scrollTo({ top: 0, behavior: 'smooth' }) }
